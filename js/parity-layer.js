...
(function () {
  ...
  function installFamilyPanel() {
    ...
    panel.querySelector('#paizomath-save-family').addEventListener('click', function(){ var name=panel.querySelector('#paizomath-child-name').value.trim(); var age=Number(panel.querySelector('#paizomath-child-age').value); var pin=panel.querySelector('#paizomath-parent-pin').value; if(name.length<2||age<6||age>18||pin.length<4){ panel.querySelector('.paizomath-family-status').textContent='Συμπλήρωσε όνομα, ηλικία 6–18 και PIN τουλάχιστον 4 χαρακτήρων.'; return; } if (!window.PAIZOMATH_CONFIGURE_PROFILE) { panel.querySelector('.paizomath-family-status').textContent='Η εφαρμογή δεν είναι ακόμη έτοιμη. Δοκίμασε ξανά.'; return; } window.PAIZOMATH_CONFIGURE_PROFILE({name:name,age:age,pin:pin}).then(function(){ localStorage.removeItem('paizomath-selected-grade'); panel.querySelector('.paizomath-family-status').textContent='Αποθηκεύτηκε και ενεργοποιήθηκε. Προτεινόμενη τάξη: '+window.PAIZOMATH_GRADE_FOR_AGE(); document.dispatchEvent(new CustomEvent('paizomath:gradechange')); }).catch(function(){ panel.querySelector('.paizomath-family-status').textContent='Το PIN πρέπει να έχει τουλάχιστον 6 χαρακτήρες.'; }); });
  }
